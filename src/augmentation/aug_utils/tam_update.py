...
def tam_update(gloss, is_segmented):
    ...
    if is_segmented:
        for i in gloss[2]:
            ...
            if matches is not None:
                index = gloss[2].index(i)
                doc = nlp(' '.join(gloss[4]))
                for token in doc:
                                ...
                                if len(gloss[3]) > index+1:
                                    ...
                                    if (matches.group()).startswith('COM'):
                                        ...
                                        if new_verb != None:
                                            ...
                                            processed_gloss.append([''.join(re.sub(re.escape(gloss[1][index]), re.sub('^x', 't', re.escape(gloss[1][index]), count=1), g1))])
                                            processed_gloss.append([''.join(re.sub(re.escape(gloss[2][index]), re.sub('COM', 'INC', re.escape(gloss[2][index]), count=1), g2))])
                                            processed_gloss.append([''.join(re.sub(re.escape(gloss[3][index]), re.sub('COM', 'INC', re.escape(gloss[3][index]), count=1), g3))])
                                            processed_gloss.append(list(map(lambda x: x.replace(orig_verb, new_verb), gloss[4])))
                                        ...
                                        if new_verb != None:
                                            ...
                                            processed_gloss.append([''.join(re.sub(re.escape(gloss[1][index]), re.sub('^t', 'x', re.escape(gloss[1][index]), count=1), g1))])
                                            processed_gloss.append([''.join(re.sub(re.escape(gloss[2][index]), re.sub('INC', 'COM', re.escape(gloss[2][index]), count=1), g2))])
                                            processed_gloss.append([''.join(re.sub(re.escape(gloss[3][index]), re.sub('INC', 'COM', re.escape(gloss[3][index]), count=1), g3))])
                                            processed_gloss.append(list(map(lambda x: x.replace(orig_verb, new_verb), gloss[4])))
                                            return processed_gloss
