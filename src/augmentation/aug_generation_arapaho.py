...
def aug_generation(
    ...
    from augmentation.aug_utils import (
        create_dataframe_arapaho,
        dataset_prep_arapaho,
        glosses_to_list,
        insert_interjection_arapaho,
        insert_noise_arapaho,
        output_dataset,
        sentence_permutations_arapaho,
    )

    # %%
    # Load in data
    df = create_dataframe_arapaho.create_dataframe(
        dataset=initial_dataset, fraction=fraction
    )
    df = df.dropna()
    ...
    if params.run_sentence_permutations:
        ...
        for gloss in glosses:
            permutations_updates.extend(
                sentence_permutations_arapaho.permute(gloss, is_segmented)
            )
        for p in permutations_updates:
            if p is not None:
                permutations_final.append(
                    dataset_prep_arapaho.dataset_prep(p, is_segmented)
                )
    ...
    interjections = [
        ["Hih'oo", "Hih'oo", "allright!", "All right !"],
        ["Yeah", "Yeah", "yeah", "Yeah"],
    ...
    if params.run_insert_interjection:
        ...
        for interjection in interjections:
            for gloss in glosses:
                interjections_final.append(
                    dataset_prep_arapaho.dataset_prep(
                        insert_interjection_arapaho.random_insert_beginning(
                            gloss, interjection, is_segmented
                        ),
                        is_segmented,
                    )
                )
    ...
    noises = [
        ["Bih'ih", "Bih'ih", "mule.deer", "Mule deer"],
    ...
    if params.run_random_insert_noise:
        ...
        for noise in noises:
            for gloss in glosses:
                noise_final.append(
                    dataset_prep_arapaho.dataset_prep(
                        insert_noise_arapaho.random_insert_beginning(
                            gloss, noise, is_segmented
                        ),
                        is_segmented,
                    )
                )
